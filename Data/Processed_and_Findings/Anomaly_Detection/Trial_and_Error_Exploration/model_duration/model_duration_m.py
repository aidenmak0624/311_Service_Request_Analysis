...
all_anomalies = []  # To store all anomalies across types
anomaly_counts = {}
...
for unique_type in unique_types:
    ...
    try:
        ...
        # Add anomaly metadata and append to the master list
        anomalies_df = anomalies.to_frame(name='Residuals')
        anomalies_df['Type'] = unique_type
        anomalies_df['Case Duration (hours)'] = df_type.loc[anomalies.index, 'Case Duration (hours)']
        anomalies_df['Open Date'] = df_type.loc[anomalies.index, 'Open Date']
        anomalies_df['Closed Date'] = df_type.loc[anomalies.index, 'Closed Date']
...
all_anomalies_df.to_csv('anomalies_data.csv', index=False)

# Convert anomaly counts to a DataFrame and save to CSV
...
anomaly_counts_df.to_csv('anomaly_counts.csv', index=False)

# Plot anomaly counts
plt.figure(figsize=(20, 8))  # Ensure correct figure size
anomaly_counts_df.plot(kind='bar', x='Type', y='Anomaly Count', title='Anomaly Counts by Type', ax=plt.gca())
plt.xlabel('Type')
